...
sigma = 550; %Cross-validated parameter values
c = 1.635;
models = SVM_multiclass_train(X_plan,y_plan,sigma,c);
modelParameters{end-1} = models;
...
X = [xi xj];
y = [ones(1,size(xi,2)) zeros(1,size(xj,2))]; %data for classification
...
for angle = 1:num_angles 
    ...
    %size
    net = constructNet(learn_rate(angle), 1, reg_param(angle), input_layer_size, 20, 2);
    net.mx = mx;
    net.sx = sx;
    net.my = my;
